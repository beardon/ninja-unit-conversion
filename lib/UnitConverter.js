...
class UnitConverter {
    ...
    hectaresToAcres (input) {
        ...
        else return hectares * this.conversionFactors.HECTARE_ACRE;
    }
    ...
    kilogramsPerHectareToPoundsPerAcre (input) {
        let kilogramsPerHectare = parseFloat(input);
        if(kilogramsPerHectare === 0 || kilogramsPerHectare < 0) return 0;
        if(isNaN(kilogramsPerHectare)) return -1;
        else return kilogramsPerHectare * this.conversionFactors.KILOGRAM_POUND / this.conversionFactors.HECTARE_ACRE;
    }
    ...
    pricePerPoundToPricePerBushel (input, conversionFactor) {
        let pricePerPound = parseFloat(input);
        let bushelCropConversionFactor = parseFloat(conversionFactor);
        if(pricePerPound === 0 || bushelCropConversionFactor === 0 || pricePerPound < 0 || bushelCropConversionFactor < 0) return 0;
        if(isNaN(pricePerPound) || isNaN(bushelCropConversionFactor)) return -1;
        else return pricePerPound * bushelCropConversionFactor;
    }
    ...
    pricePerKilogramToPricePerBushel (input, conversionFactor) {
        let pricePerKilogram = parseFloat(input);
        let bushelCropConversionFactor = parseFloat(conversionFactor);
        if(pricePerKilogram === 0 || bushelCropConversionFactor === 0 || pricePerKilogram < 0 || bushelCropConversionFactor < 0) return 0;
        if(isNaN(pricePerKilogram) || isNaN(bushelCropConversionFactor)) return -1;
        else {
            let pricePerPound = this.pricePerKilogramToPricePerPound(pricePerKilogram);
            return this.pricePerPoundToPricePerBushel(pricePerPound, bushelCropConversionFactor);
        }
    }

    pricePerKilogramToPricePerPound (input) {
        let pricePerKilogram = parseFloat(input);
        if(pricePerKilogram === 0 || pricePerKilogram < 0) return 0;
        if(isNaN(pricePerKilogram)) return -1;
        else return pricePerKilogram / this.conversionFactors.KILOGRAM_POUND;
    }
